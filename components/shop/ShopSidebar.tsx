...
export function ShopSidebar({ params }: { params: Params }) {
  ...
  const content = (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 lg:sticky lg:top-24">
      <h2 className="hidden lg:block text-base font-bold text-gray-900 mb-5" style={{ fontFamily: "'Poppins', sans-serif" }}>
        Filters
      </h2>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col gap-1">
          {[
            ...
            <Link
              key={g.value}
              href={buildHref(params, { gender: g.value || undefined, page: '1' })}
              onClick={close}
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col gap-1">
          {[
            ...
            <Link
              key={s.value}
              href={buildHref(params, { sort: s.value, page: '1' })}
              onClick={close}
              className={`px-3 py-2 text-sm font-bold rounded-xl transition-all ${
      ...
      <div className="lg:hidden">
        <button
          ...
          className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl border shadow-sm text-sm font-bold transition-all ${
            hasFilters
              ? 'bg-[#FFF8E1] border-[#F5C000] text-gray-900'
        ...
        >
          <span className="flex items-center gap-2">
            ...
            Filters
            {hasFilters && (
              <span className="px-1.5 py-0.5 bg-[#D9247A] text-white text-[10px] rounded-full">Active</span>
            )}
          </span>
